...
def dashboard():
    if ('user' in session and session['user']==params['admin_user']):
        ...
        return render_template('ad_dashboard.html', params=params, posts=posts)
...
def edit(sno):
    if ('user' in session and session['user']==params['admin_user']):
        if request.method=="POST":
            ...
            if sno=="0":
                post=Posts(title=box_title, slug=box_slug, content=box_content, subtitle=box_subtitle, img_file=box_img_file, date=box_date)
                db.session.add(post)
                db.session.commit()
            else:
